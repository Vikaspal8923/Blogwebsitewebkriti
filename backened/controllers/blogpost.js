...
exports.updateblogpost = async(req,res)=>{
    ...
    try{
         ...
         if(!userresult){
              ...
              const blogpostresult = await Blogpost.findByIdAndUpdate(postid , { title:updatedTitle ,content:updatedContent,updatedat:Date.now()},{new:true} );
...
exports.getRandomBlogPosts = async (req, res) => {
    try {
      console.log("Get random blogs API start");
      ...
      // console.log('Before random blogpost result:', randomBlogs);
      ...
      // console.log('After random blogpost result');
  ...
  exports.getallpostbyalluser = async (req, res) => {
    try {
       
        const totalBlogs = await Blogpost.countDocuments();

      
        const randomBlogs = await Blogpost.aggregate([
            { $sample: { size: totalBlogs } } 
        ]);

        res.status(200).json({
            success: true,
          message:" all blog fetch successfully",randomBlogs
        });
    } catch (error) {
        res.status(500).json({
            success: false,

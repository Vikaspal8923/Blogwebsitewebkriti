...
async function Login(email,password){
  ...
  try{
                ...
                localStorage.setItem("user", JSON.stringify(response.data.user))
                    ...
                    localStorage.setItem("newtoken", JSON.stringify(response.data.token))
                       console.log(" new token of login is :",newtoken)
...
function logout() {
    ...
    console.log(" new token of logout is :",newtoken)
...
let fetchtrending = false;
let fetchrandom = false;
...
async function handleLatestBlogs() {
  ...
  console.log("Latest blogs:", latestblogsarray); 
              ...
              if(fetchlatest && fetchtrending && fetchrandom){
                                          
                document.querySelector('.main-page-loading').style.display = "none";
                document.querySelector('.vp-main-page').style.display = "block";
...
document.getElementById('lat1-p').addEventListener("click", function() {
  ...
  window.location.href = `previewblog.html?postid=${encodedPostId}`;
               ...
               toastr.success('please Login to go further','Success')
}
  }); 
...
async function gettrending() {
  ...
  try{
      ...
      trendingblogsarray = response.data.trendingBlogs ;//93636+
  ...
  console.log("trending blogs:", featureblogarray); 
              ...
              //  document.getElementById('pf3-n').innerText = `By ${featureblogarray[2].author.firstname+" "+ featureblogarray[2].author.lastname } `
                    ...
                    fetchrandom = true;
                           ...
                           console.log("fetchtrending:", fetchtrending,"fetchlatest:",fetchlatest,"fetchrandom:",fetchrandom);
                    if(fetchlatest && fetchtrending && fetchrandom){
...
document.getElementById('f1-p').addEventListener("click", function() {
  ...
  document.getElementById('f2-p').addEventListener("click", function() {
    console.log("token and istoken:",token);
  ...
  document.getElementById('f5-p').addEventListener("click", function() {
    ...
    const encodedPostId = encodeURIComponent(featureblogarray[4]._id);
    if( newtoken  &&   !isTokenExpired(newtoken) ){
